...
export async function GET(request: Request) {
	try {
		...
		const sanitizedDocs: Record<string, boolean | null | number | string | undefined>[] = unpaidUsers.map(payloadParseUserForExport);

		//
		// Convert to CSV and return the response

		const csvData = Papa.unparse(sanitizedDocs, { header: true });
		const filename = `users-export-unpaid-${Date.now()}.csv`;
