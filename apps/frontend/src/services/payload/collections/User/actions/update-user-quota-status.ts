...
export async function updateUserQuotaStatus(userId: string) {
	...
	for (const quotaData of allUserQuotas) {
		...
		try {
			...
			LOGGER.error('mollie-update-quota-status', `Failed to fetch the payment data from Mollie for user NIF "${userData.tax_id}" for the quota year "${quotaData.year}". Error: ${error.message}`);
			continue;
		}
		//
	}
