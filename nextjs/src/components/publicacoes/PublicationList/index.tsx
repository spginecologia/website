...
export default function PublicationList({ publications }: { publications: Publication[] }) {
    return <Grid columns="abcd" gap="md">
        {publications.map((publication) => (
            <Card key={publication.id} style={{ height: "100%" }} link={(publication.publication_file as Media)?.url || "/placeholder.png"} target="_blank" image={{ src: (publication.featured as Media)?.url || "", alt: publication.title, size: 400 }}>
