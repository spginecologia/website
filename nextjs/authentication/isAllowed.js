    ...
    if (permissions.length) {
      for (const permission of permissions) {
        ...
        if (session.user.permissions[permission.scope][permission.action].is_allowed !== true) {
          throw new Error(`Permission denied for user "${session?.user?.email}" | Scope: ${permission.scope} | Action: ${permission.action}`);
        }
          ...
          for (const field of permission.fields) {
            for (const value of field.values) {
              if (!session.user.permissions[permission.scope][permission.action].fields[field.key]?.includes(value)) {
                throw new Error(`Permission denied for user "${session?.user?.email}" | Scope: ${permission.scope} | Action: ${permission.action} | Field: ${field.key} | Value: ${value}`);
              }
            }
          }
        }
