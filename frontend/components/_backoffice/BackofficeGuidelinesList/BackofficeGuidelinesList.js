...
export default function BackofficeGuidelinesList() {
	...
	const handleCreate = async () => {
		try {
			setIsCreating(true);
			const result = await API({ method: 'GET', operation: 'create', service: 'guidelines' });
	...
	return (
		<BackofficeWrapperList isCreating={isCreating} isLoading={allGuidelinesLoading} onChangeSearchQuery={setSearchQuery} onCreate={handleCreate} searchQuery={searchQuery}>
			{filteredGuidelinesData && filteredGuidelinesData.length > 0 ? filteredGuidelinesData.map(item => <BackofficeWrapperListItem key={item._id} isSelected={guideline_id === item._id} onClick={() => handleOpen(item._id)} title={item.title} />) : <NoDataLabel fill />}
