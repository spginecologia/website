...
export default async function handler(req, res) {
  ...
  try {
    req.body = LinkValidation.cast(req.body);
  } catch (err) {
    console.log(err);
  ...
  try {
    const editedDocument = await LinkModel.replaceOne({ _id: { $eq: req.query._id } }, req.body, { new: true });
    if (!editedDocument) return await res.status(404).json({ message: `Link with _id: ${req.query._id} not found.` });
    else return await res.status(200).json(editedDocument);
  } catch (err) {
    console.log(err);
    return await res.status(500).json({ message: 'Cannot update this Link.' });
  }
